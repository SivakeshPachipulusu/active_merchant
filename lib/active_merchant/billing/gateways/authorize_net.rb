  ...
  module Billing
    class AuthorizeNetGateway < Gateway
      ...
      def verify(payment_method, options = {})
        ...
        return amount if amount.class == Response
      ...
      def define_amount_for_verify(payment_method, options)
        if options[:verify_amount].present?
          if options[:verify_amount].class != Integer || options[:verify_amount] < 0
            ...
            response = 0
            response = Response.new(false, 'zip and address must be different than blank value') unless validate_billing_address_values?(options) 
          else
            response = options[:verify_amount]
          end
        else
